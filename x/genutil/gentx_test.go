...
import (
	...
	genutiltestutil "github.com/axiome-pro/axm-node/x/genutil/testutil"
...
func (suite *GenTxTestSuite) SetupTest() {
	...
	suite.msg1, err = stakingtypes.NewMsgCreateValidator(
		sdk.ValAddress(pk1.Address()).String(), pk1, amount, desc)
	suite.NoError(err)
	suite.msg2, err = stakingtypes.NewMsgCreateValidator(
		sdk.ValAddress(pk2.Address()).String(), pk1, amount, desc)
	suite.NoError(err)
}
