...
import (
	...
	"github.com/axiome-pro/axm-node/x/staking"
...
func TestValidateGenesisMultipleMessages(t *testing.T) {
	...
	msg1, err := stakingtypes.NewMsgCreateValidator(sdk.ValAddress(pk1.Address()).String(), pk1, sdk.NewInt64Coin(sdk.DefaultBondDenom, 50), desc)
	require.NoError(t, err)

	msg2, err := stakingtypes.NewMsgCreateValidator(sdk.ValAddress(pk2.Address()).String(), pk2,
		sdk.NewInt64Coin(sdk.DefaultBondDenom, 50), desc)
	require.NoError(t, err)
...
func TestValidateGenesisBadMessage(t *testing.T) {
	...
	msg1 := stakingtypes.NewMsgEditValidator(sdk.ValAddress(pk1.Address()).String(), desc)
