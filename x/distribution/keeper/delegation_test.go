...
func TestCalculateRewardsBasic(t *testing.T) {
	...
	accountKeeper.EXPECT().AddressCodec().Return(address.NewBech32Codec(params.Bech32PrefixAccAddr)).AnyTimes()
	...
	// rewards should be 99.97%by default
	require.Equal(t, sdk.DecCoins{{Denom: sdk.DefaultBondDenom, Amount: math.LegacyNewDecWithPrec(997, 2)}}, rewards)

	// commission should be the other part
...
func TestCalculateRewardsAfterSlash(t *testing.T) {
	...
	require.Equal(t, sdk.DecCoins{{Denom: sdk.DefaultBondDenom, Amount: math.LegacyNewDec(9970000)}}, rewards)

	// commission should be the other half
	valCommission, err := distrKeeper.GetValidatorAccumulatedCommission(ctx, valAddr)
	require.NoError(t, err)
	require.Equal(t, sdk.DecCoins{{Denom: sdk.DefaultBondDenom, Amount: math.LegacyNewDec(30000)}},
...
func TestCalculateRewardsAfterManySlashes(t *testing.T) {
	...
	params.ValidatorCommissionRate = math.LegacyNewDecWithPrec(5, 1)
	require.NoError(t, distrKeeper.Params.Set(ctx, params))

	// rewards should be half the tokens
	require.Equal(t, sdk.DecCoins{{Denom: sdk.DefaultBondDenom, Amount: math.LegacyNewDec(19940000)}}, rewards)

	// commission should be the other half
	valCommission, err := distrKeeper.GetValidatorAccumulatedCommission(ctx, valAddr)
	require.NoError(t, err)
	require.Equal(t, sdk.DecCoins{{Denom: sdk.DefaultBondDenom, Amount: math.LegacyNewDec(60000)}},
		valCommission.Commission)
}
...
func TestCalculateRewardsMultiDelegator(t *testing.T) {
	...
	// rewards for del0 should be 3/4 initial + emission
	require.Equal(t, sdk.DecCoins{{Denom: sdk.DefaultBondDenom, Amount: math.LegacyNewDecWithPrec(2991, 2)}}, rewards)

	// calculate delegation rewards for del2
	rewards, err = distrKeeper.CalculateDelegationRewards(ctx, val, del1, endingPeriod)
	require.NoError(t, err)

	// rewards for del2 should be 1/4 initial + emission
	require.Equal(t, sdk.DecCoins{{Denom: sdk.DefaultBondDenom, Amount: math.LegacyNewDecWithPrec(997, 2)}}, rewards)

	// commission should be equal to initial (50% twice)
	valCommission, err := distrKeeper.GetValidatorAccumulatedCommission(ctx, valAddr)
	require.NoError(t, err)
	require.Equal(t, sdk.DecCoins{{Denom: sdk.DefaultBondDenom, Amount: math.LegacyNewDecWithPrec(12, 2)}}, valCommission.Commission)
}
...
func TestWithdrawDelegationRewardsBasic(t *testing.T) {
	...
	accountKeeper.EXPECT().GetModuleAddress("distribution").Return(distrAcc.GetAddress())
	stakingKeeper.EXPECT().ValidatorAddressCodec().Return(address.NewBech32Codec(params.Bech32PrefixValAddr)).AnyTimes()
	stakingKeeper.EXPECT().MaximumMonthlyPoints(gomock.Any()).Return(stakingtypes.DefaultMaximumMonthlyPoints, nil).AnyTimes()
	accountKeeper.EXPECT().AddressCodec().Return(address.NewBech32Codec(params.Bech32PrefixAccAddr)).AnyTimes()
	stakingKeeper.EXPECT().ValidatorEmissionRate(gomock.Any()).Return(math.LegacyZeroDec(), nil).AnyTimes()
	stakingKeeper.EXPECT().BondDenom(gomock.Any()).Return(params.DefaultBondDenom, nil).AnyTimes()
	...
	expRewards := sdk.Coins{sdk.NewCoin(sdk.DefaultBondDenom, math.NewInt(9970000))}
	...
	expCommission := sdk.Coins{sdk.NewCoin(sdk.DefaultBondDenom, math.NewInt(30000))}
...
func TestCalculateRewardsAfterManySlashesInSameBlock(t *testing.T) {
	...
	// rewards should be tokens + emission
	require.Equal(t, sdk.DecCoins{{Denom: sdk.DefaultBondDenom, Amount: math.LegacyNewDec(19940000)}}, rewards)

	// commission should be the other half
	valCommission, err := distrKeeper.GetValidatorAccumulatedCommission(ctx, valAddr)
	require.NoError(t, err)
	require.Equal(t, sdk.DecCoins{{Denom: sdk.DefaultBondDenom, Amount: math.LegacyNewDec(60000)}}, valCommission.Commission)
}
...
func TestCalculateRewardsMultiDelegatorMultiSlash(t *testing.T) {
	...
	require.Equal(t, sdk.DecCoins{{Denom: sdk.DefaultBondDenom, Amount: math.LegacyNewDec(39880000)}}, rewards)
	...
	valCommission, err := distrKeeper.GetValidatorAccumulatedCommission(ctx, valAddr)
	require.NoError(t, err)
	require.Equal(t, sdk.DecCoins{{Denom: sdk.DefaultBondDenom, Amount: math.LegacyNewDec(180000)}}, valCommission.Commission)
}
...
func TestCalculateRewardsMultiDelegatorMultWithdraw(t *testing.T) {
	...
	expRewards := sdk.Coins{sdk.NewCoin(sdk.DefaultBondDenom, math.NewInt(39))}
	bankKeeper.EXPECT().SendCoinsFromModuleToAccount(ctx, disttypes.ModuleName, addr, expRewards)
	_, err = distrKeeper.WithdrawDelegationRewards(ctx, addr, valAddr)
	require.NoError(t, err)

	// second delegator withdraws
	expRewards = sdk.Coins{sdk.NewCoin(sdk.DefaultBondDenom, math.NewInt(9))}
	...
	expCommission := sdk.Coins{sdk.NewCoin(sdk.DefaultBondDenom, math.NewInt(9))}
	bankKeeper.EXPECT().SendCoinsFromModuleToAccount(gomock.Any(), disttypes.ModuleName, addr, expCommission)
	_, err = distrKeeper.WithdrawValidatorCommission(ctx, valAddr)
	require.NoError(t, err)
	...
	valCommission, err := distrKeeper.GetValidatorAccumulatedCommission(ctx, valAddr)
	require.NoError(t, err)
	require.Equal(t, sdk.DecCoins{sdk.NewDecCoinFromDec(sdk.DefaultBondDenom, math.LegacyNewDecWithPrec(12, 2))}, valCommission.Commission)
	...
	valCommission, err = distrKeeper.GetValidatorAccumulatedCommission(ctx, valAddr)
	require.NoError(t, err)
	require.Equal(t, sdk.DecCoins{{Denom: sdk.DefaultBondDenom, Amount: math.LegacyNewDecWithPrec(18, 2)}}, valCommission.Commission)
	...
	require.Equal(t, sdk.DecCoins{{Denom: sdk.DefaultBondDenom, Amount: math.LegacyNewDecWithPrec(1994, 2)}}, rewards)

	valCommission, err = distrKeeper.GetValidatorAccumulatedCommission(ctx, valAddr)
	require.NoError(t, err)
	require.Equal(t, sdk.DecCoins{{Denom: sdk.DefaultBondDenom, Amount: math.LegacyNewDecWithPrec(24, 2)}}, valCommission.Commission)
}
...
func Test100PercentCommissionReward(t *testing.T) {
	...
	bankKeeper.EXPECT().SendCoinsFromModuleToAccount(gomock.Any(), disttypes.ModuleName, addr, gomock.Any()).AnyTimes()
	...
	emissionRewards := sdk.Coins{sdk.NewCoin(sdk.DefaultBondDenom, math.NewInt(79))}
	require.Equal(t, emissionRewards.String(), rewards.String())
	...
	for _, attr := range lastEvent.Attributes {
		if attr.Key == "amount" && attr.Value == "79stake" {
			hasValue = true
		}
	}
