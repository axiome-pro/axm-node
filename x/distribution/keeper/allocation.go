...
func (k Keeper) AllocateTokens(ctx context.Context, totalPreviousPower int64, bondedVotes []abci.VoteInfo) error {
	...
	// burn percent coins from fee
	burnRate, err := k.GetBurnRate(ctx)
	if err != nil {
		return err
	}

	feesToBurn := feesCollected.MulDecTruncate(burnRate)
	if !feesToBurn.IsZero() {
		feesToBurnInt := sdk.NewCoins()
		for _, coin := range feesToBurn {
			feesToBurnInt = feesToBurnInt.Add(sdk.NewCoin(coin.Denom, coin.Amount.TruncateInt()))
		}

		err = k.bankKeeper.BurnCoins(ctx, types.ModuleName, feesToBurnInt)
		if err != nil {
			return err
		}

		feesCollected = feesCollected.Sub(sdk.NewDecCoinsFromCoins(feesToBurnInt...))
	}
	...
	// allocate tokens proportionally to vote power
	//
...
func (k Keeper) AllocateTokensToValidator(ctx context.Context, val stakingtypes.ValidatorI, tokens sdk.DecCoins) error {
	...
	commissionRate, err := k.GetValidatorCommissionRate(ctx)
	if err != nil {
		return err
	}
