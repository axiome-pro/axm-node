...
func (k Keeper) calculateDelegationEmission(ctx context.Context, val stakingtypes.ValidatorI, del stakingtypes.DelegationI) (rewards math.LegacyDec, err error) {

	maxPoints, err := k.stakingKeeper.MaximumMonthlyPoints(ctx)
	...
	tokens = tokens.MulInt64(int64(val.GetPoints() - del.GetPoints())).QuoInt64(int64(maxPoints))

	rate, err := k.stakingKeeper.ValidatorEmissionRate(ctx)
	if err != nil {
		return math.LegacyDec{}, err
	}

	tokens = tokens.Sub(tokens.Mul(rate))

	return tokens, nil
}

func (k Keeper) CalculateDelegationEmission(ctx context.Context, val stakingtypes.ValidatorI, del stakingtypes.DelegationI) (rewards sdk.DecCoins, err error) {
	...
	tokens, err := k.calculateDelegationEmission(ctx, val, del)

	if err != nil {
		return sdk.DecCoins{}, err
	}

	rewards = rewards.Add(sdk.NewDecCoinFromDec(bondDenom, tokens))
...
func (k Keeper) CalculateDelegationRewards(ctx context.Context, val stakingtypes.ValidatorI, del stakingtypes.DelegationI, endingPeriod uint64) (rewards sdk.DecCoins, err error) {
	...
	// Calculate emission based rewards
...
func (k Keeper) withdrawDelegationRewards(ctx context.Context, val stakingtypes.ValidatorI, del stakingtypes.DelegationI) (sdk.Coins, error) {
	...
	// Get base denom for emissionDec and event
	baseDenom, _ := sdk.GetBaseDenom()
	if baseDenom == "" {
		baseDenom = sdk.DefaultBondDenom
	}

	// Calculate and add emissionDec to finalRewards (decimal part are truncated i.e. burned)
	emissionDec, err := k.calculateDelegationEmission(ctx, val, del)
	if err != nil {
		return nil, err
	}

	emission := sdk.NewCoin(baseDenom, emissionDec.TruncateInt())

	if !emission.IsZero() {
		err = k.bankKeeper.MintCoins(ctx, types.ModuleName, sdk.NewCoins(emission))
		if err != nil {
			return nil, err
		}

		err = k.stakingKeeper.UpdateEmissionPoints(ctx, del, val.GetPoints())
		if err != nil {
			return nil, err
		}
	}
	...
	if !finalRewards.IsZero() || !emission.IsZero() {
		withdrawAddr, err := k.GetDelegatorWithdrawAddr(ctx, delAddr)
		if err != nil {
			return nil, err
		}

		err = k.bankKeeper.SendCoinsFromModuleToAccount(ctx, types.ModuleName, withdrawAddr, finalRewards.Add(emission))
		if err != nil {
			return nil, err
		}
	}
