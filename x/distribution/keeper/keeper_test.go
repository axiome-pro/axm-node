...
func TestSetWithdrawAddr(t *testing.T) {
	...
	par.WithdrawAddrEnabled = false
	require.NoError(t, distrKeeper.Params.Set(ctx, par))

	err := distrKeeper.SetWithdrawAddr(ctx, delegatorAddr, withdrawAddr)
	require.NotNil(t, err)

	par.WithdrawAddrEnabled = true
	require.NoError(t, distrKeeper.Params.Set(ctx, par))
...
func TestWithdrawValidatorCommission(t *testing.T) {
	...
	val0, err := distrtestutil.CreateValidator(valConsPk0, math.NewInt(20_000_000_000))
	require.NoError(t, err)

	valAddr := sdk.ValAddress(val0.GetOperator())
	...
	stakingKeeper.EXPECT().Validator(gomock.Any(), valAddr).Return(val0, nil).AnyTimes()
	...
	bankKeeper.EXPECT().SendCoinsFromModuleToAccount(gomock.Any(), "distribution", accAddr, coins).Return(nil)
