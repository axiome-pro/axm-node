...
func (k Querier) DelegationRewards(ctx context.Context, req *types.QueryDelegationRewardsRequest) (*types.QueryDelegationRewardsResponse, error) {
	...
	emitted, err := k.CalculateDelegationEmission(ctx, val, del)
	if err != nil {
		return nil, err
	}

	return &types.QueryDelegationRewardsResponse{Rewards: rewards, Emitted: emitted}, nil
}
...
func (k Querier) DelegationTotalRewards(ctx context.Context, req *types.QueryDelegationTotalRewardsRequest) (*types.QueryDelegationTotalRewardsResponse, error) {
	...
	err = k.stakingKeeper.IterateDelegations(
		...
		func(_ int64, del stakingtypes.DelegationI) (stop bool) {
			...
			delEmission, err := k.CalculateDelegationEmission(ctx, val, del)
			if err != nil {
				panic(err)
			}

			delRewards = append(delRewards, types.NewDelegationDelegatorReward(del.GetValidatorAddr(), delReward, delEmission))
			total = total.Add(delReward...)
			total = total.Add(delEmission...)
			return false
		},
	)
	if err != nil {
