...
func (k Keeper) Delegate(
	...
	oldCoins := math.ZeroInt()

	if err == nil {
		// found
		err = k.Hooks().BeforeDelegationSharesModified(ctx, delAddr, valbz)
		oldCoins = oldCoins.Add(validator.TokensFromSharesTruncated(delegation.GetShares()).TruncateInt())
	...
	if subtractAccount {
		...
		// Check if delegation available for delegator
		err = k.RefHooks().CheckDelegationAvailable(ctx, delAddrStr, validator.GetOperator())
		if err != nil {
			return math.LegacyZeroDec(), err
		}
		...
		bondAmt, err = k.RefHooks().SpendCoinsForRef(ctx, delAddrStr, bondAmt)
		if err != nil {
			return math.LegacyZeroDec(), err
		}
	...
	err = k.RefHooks().DelegationCoinsModified(ctx, delAddrStr, validator.GetOperator(), oldCoins, oldCoins.Add(bondAmt))
	if err != nil {
		return math.LegacyDec{}, err
	}
	...
	// Set new emission points for delegation
	delegation.Points = validator.Points
...
func (k Keeper) UpdateEmissionPoints(ctx context.Context, del types.DelegationI, points uint64) error {
	valAddr, valErr := k.validatorAddressCodec.StringToBytes(del.GetValidatorAddr())
	if valErr != nil {
		return sdkerrors.ErrInvalidAddress.Wrapf("invalid validator address: %s", valErr)
	}

	delegatorAddress, err := k.authKeeper.AddressCodec().StringToBytes(del.GetDelegatorAddr())
	if err != nil {
...
func (k Keeper) DecrementEmission(ctx context.Context, valAddr sdk.ValAddress, amount math.LegacyDec) error {
	...
	validator.Emission = validator.Emission.Sub(amount)

	if validator.Emission.IsNegative() {
		return sdkerrors.ErrInvalidCoins.Wrapf("emission decrement more then current validator emission %s", validator.GetOperator())
	}
...
func (k Keeper) Unbond(
	...
	oldCoins := validator.TokensFromSharesTruncated(delegation.Shares).TruncateInt()
	...
	err = k.RefHooks().DelegationCoinsModified(ctx, delegation.DelegatorAddress, delegation.ValidatorAddress, oldCoins, oldCoins.Sub(amount))
...
func (k Keeper) getBeginInfo(
	...
	sdkCtx := sdk.UnwrapSDKContext(ctx)
	unbondingTime, err := k.RedelegationTime(ctx)
	if err != nil {
		return
	}
	...
	for i := 0; i < len(ubd.Entries); i++ {
		...
		if entry.IsMature(ctxTime) && !entry.OnHold() {
			...
			if !entry.Balance.IsZero() {
				// TODO: get burn percent from params
				burnPercent := math.NewInt(10)

				toBurn := entry.Balance.Mul(burnPercent).Quo(math.NewInt(100))
				amt := sdk.NewCoin(bondDenom, entry.Balance.Sub(toBurn))
				burnAmt := sdk.NewCoin(bondDenom, toBurn)

				// Undelegate coins to delegators account
				...
				if err := k.bankKeeper.BurnCoins(
					ctx, types.NotBondedPoolName, sdk.NewCoins(burnAmt),
				); err != nil {
					return nil, err
				}

				balances = balances.Add(amt.Add(burnAmt))
			}
		}
	}
