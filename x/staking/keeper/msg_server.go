...
func (k msgServer) CreateValidator(ctx context.Context, msg *types.MsgCreateValidator) (*types.MsgCreateValidatorResponse, error) {
	...
	minSelfDelegation, err := k.Keeper.MinSelfDelegation(ctx)

	if err != nil {
		return nil, err
	}

	if err := msg.Validate(k.validatorAddressCodec, minSelfDelegation); err != nil {
		return nil, err
	}
	...
	shares, err := k.Keeper.Delegate(ctx, sdk.AccAddress(valAddr), msg.Value.Amount, types.Unbonded, validator, true)
	if err != nil {
		return nil, err
	}

	// Check if now delegated less than minSelfDelegation module param (check at this time again
	// because other modules can take part of delegated amount)
	validator, err = k.Keeper.GetValidator(ctx, valAddr)
	if err != nil {
		return nil, err
	}

	delegatedTokens := validator.TokensFromSharesTruncated(shares).TruncateInt()

	if delegatedTokens.LT(minSelfDelegation) {
		return nil, errors.Wrapf(sdkerrors.ErrInsufficientFunds, "Total self delegated %d is less than minSelfDelegation %d", delegatedTokens.Int64(), minSelfDelegation.Int64())
	}
