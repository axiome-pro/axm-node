...
import (
	...
	"github.com/axiome-pro/axm-node/x/staking/simulation"
...
func (s *SimTestSuite) SetupTest() {
	...
	accs := []appsimtestutil.GenesisAccount{
	...
	startupCfg := appsimtestutil.DefaultStartUpConfig()
	...
	app, err := appsimtestutil.SetupWithConfiguration(cfg, startupCfg, &s.txConfig, &bankKeeper, &accountKeeper, &distrKeeper, &stakingKeeper)
	require.NoError(s.T(), err)
	...
	require.NoError(s.T(), err)
	params.MinSelfDelegation = math.NewInt(1)
	require.NoError(s.T(), stakingKeeper.SetParams(ctx, params))
	...
	for _, account := range accounts[0:] {
...
func (s *SimTestSuite) getTestingValidator0(ctx sdk.Context) types.Validator {
	return s.getTestingValidator(ctx, 1)
}

func (s *SimTestSuite) getTestingValidator1(ctx sdk.Context) types.Validator {
	return s.getTestingValidator(ctx, 2)
}

func (s *SimTestSuite) getTestingValidator(ctx sdk.Context, n int) types.Validator {
