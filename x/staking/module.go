...
import (
	...
	modulev1 "github.com/axiome-pro/axm-node/api/axiome/staking/module/v1"
	"cosmossdk.io/core/appmodule"
	"cosmossdk.io/core/store"
	"cosmossdk.io/depinject"

	"github.com/axiome-pro/axm-node/x/staking/client/cli"
	"github.com/axiome-pro/axm-node/x/staking/exported"
	"github.com/axiome-pro/axm-node/x/staking/keeper"
...
func init() {
	appmodule.Register(
		...
		appmodule.Invoke(InvokeSetRefStakingHooks),
	)
}
...
func InvokeSetRefStakingHooks(
	config *modulev1.Module,
	keeper *keeper.Keeper,
	stakingHooks map[string]types.RefStakingHooksWrapper,
) error {
	// all arguments to invokers are optional
	if keeper == nil || config == nil {
		return nil
	...
	var multiHooks types.MultiRefStakingHooks
	for _, modName := range order {
		hook, ok := stakingHooks[modName]
		if !ok {
			return fmt.Errorf("can't find staking hooks for module %s", modName)
