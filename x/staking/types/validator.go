...
// ValidatorsByvotePower implements sort.Interface for []Validator based on
// the votePower and Address fields.
// The validators are sorted first by their vote power (descending). Secondary index - Address (ascending).
// Copied from tendermint/types/validator_set.go
type ValidatorsByvotePower []Validator

func (valz ValidatorsByvotePower) Len() int { return len(valz) }
...
func (v *Validator) MinEqual(other *Validator) bool {
	return v.OperatorAddress == other.OperatorAddress &&
		...
		v.Points == other.Points &&
		v.Emission.Equal(other.Emission)
}
...
func (v Validator) GetPoints() uint64 { return v.Points }
...
func (v Validator) GetEmission() math.LegacyDec        { return v.Emission }
